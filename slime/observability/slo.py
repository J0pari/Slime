...
class SLOChecker:
    ...
    def check_slo(self, slo: SLO, metric_value: float) -> SLOStatus:
        ...
        if violation:
            ...
            logger.warning(f'SLO violation: {slo.name} - metric={metric_value:.4f}, threshold={slo.threshold}')
        window_start = current_time - slo.window_size
        self._violations[slo.name] = [t for t in self._violations[slo.name] if t >= window_start]
    ...
    def get_error_budget_burn_rate(self, slo_name: str) -> float:
        ...
        current_time = time.time()
        window_start = current_time - slo.window_size
        recent_violations = [t for t in self._violations[slo_name] if t >= window_start]
    ...
    def stats(self) -> Dict:
        ...
        for slo in self.slos:
            current_time = time.time()
            window_start = current_time - slo.window_size
            recent_violations = [t for t in self._violations[slo.name] if t >= window_start]
            violation_rate = len(recent_violations) / max(1, slo.window_size)
            ...
            results[slo.name] = {'violations_in_window': len(recent_violations), 'violation_rate': violation_rate, 'error_budget_burn_rate': burn_rate, 'remaining_error_budget': remaining_budget, 'last_check': self._last_check[slo.name], 'threshold': slo.threshold, 'comparison': slo.comparison}
        return results
...
def create_default_slos() -> List[SLO]:
    return [SLO(name='latency_p95', metric_name='inference_latency_ms', threshold=100.0, comparison='<', window_size=3600, error_budget=0.05), SLO(name='latency_p99', metric_name='inference_latency_ms', threshold=200.0, comparison='<', window_size=3600, error_budget=0.01), SLO(name='memory_limit', metric_name='memory_allocated_mb', threshold=8000.0, comparison='<', window_size=1800, error_budget=0.1), SLO(name='pool_size_min', metric_name='pool_size', threshold=1.0, comparison='>=', window_size=600, error_budget=0.0), SLO(name='archive_coverage', metric_name='archive_coverage', threshold=0.1, comparison='>=', window_size=3600, error_budget=0.2)]