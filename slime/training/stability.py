"""Phased training protocol for gradual lifecycle enablement"""
...
class TrainingPhase(Enum):
    """Training phases with increasing dynamics"""
    WARMUP = "warmup"
    GENTLE = "gentle"
    FULL = "full"
...
class StabilityManager:
    """Manage phased training to prevent instability from dynamic lifecycle"""

    def __init__(self, warmup_steps: int = 1000, gentle_steps: int = 5000):
        """Initialize stability manager.

        Args:
            warmup_steps: Steps before allowing any lifecycle changes
            gentle_steps: Steps before allowing full dynamics
        """
        ...
        self.phases = [
            PhaseConfig(
                name=TrainingPhase.WARMUP,
                min_step=0,
                max_step=warmup_steps,
                allow_births=False,
                allow_deaths=False,
                allow_culling=False,
                pool_size_limit=None,
                description="Static pool, no lifecycle changes, establish baseline"
            ),
            PhaseConfig(
                name=TrainingPhase.GENTLE,
                min_step=warmup_steps,
                max_step=gentle_steps,
                allow_births=True,
                allow_deaths=False,
                allow_culling=False,
                pool_size_limit=64,
                description="Allow births but no deaths, controlled growth"
            ),
            PhaseConfig(
                name=TrainingPhase.FULL,
                min_step=gentle_steps,
                max_step=None,
                allow_births=True,
                allow_deaths=True,
                allow_culling=True,
                pool_size_limit=None,
                description="Full dynamics enabled"
            ),
        ]
    ...
    def get_phase(self, step: int) -> PhaseConfig:
        """Get current training phase for given step"""
    ...
    def _on_phase_change(self, new_phase: PhaseConfig, step: int) -> None:
        ...
        if self._current_phase:
            logger.info(
                ...
                f"{self._current_phase.name.value} -> {new_phase.name.value}"
            )
    ...
    def stats(self) -> dict:
        ...
        return {
            'current_phase': self._current_phase.name.value if self._current_phase else None,
            'phase_changes': len(self._phase_change_steps),
            'warmup_steps': self.warmup_steps,
            'gentle_steps': self.gentle_steps,
        }


def create_default_stability_manager() -> StabilityManager:
    """Create stability manager with default configuration"""
