"""Lifecycle safety guardrails for training stability"""
...
class LifecycleConfig:
    """Configuration for lifecycle safety guardrails"""
...
class LifecycleManager:
    """Manage lifecycle safety to prevent training instability.

    Implements hard limits and loss gates per Invariant #9.
    """
    ...
    def update_loss_ema(self, current_loss: float) -> None:
        ...
        if self._loss_ema is None:
            ...
            self._loss_ema = (
                self.config.loss_ema_alpha * current_loss +
                (1 - self.config.loss_ema_alpha) * self._loss_ema
            )
    ...
    def check_loss_divergence(self, current_loss: float) -> bool:
        ...
        Returns:
            True if loss is diverging beyond acceptable ratio
        """
        if self._loss_ema is None:
            return False
        ...
        if ratio > self.config.max_loss_ratio:
            logger.warning(
                f"Loss divergence detected: "
                f"current={current_loss:.4f}, ema={self._loss_ema:.4f}, ratio={ratio:.2f}"
            )
            return True
    ...
    def check_pool_size_limit(self, pool_size: int) -> bool:
        """Check if pool size exceeds hard limit.

        Returns:
            True if pool size exceeds limit
        """
        if pool_size > self.config.max_pool_size:
            logger.warning(
                f"Pool size limit exceeded: "
                f"current={pool_size}, max={self.config.max_pool_size}"
            )
            return True
        return False

    def check_archive_size_limit(self, archive_size: int) -> bool:
        """Check if archive size exceeds hard limit.

        Returns:
            True if archive size exceeds limit
        """
        if archive_size > self.config.max_archive_size:
            logger.warning(
                f"Archive size limit exceeded: "
                f"current={archive_size}, max={self.config.max_archive_size}"
            )
            return True
        return False
    ...
    def step(
        ...
        """Perform lifecycle safety check.
        ...
        results = {
            ...
            'violations': [],
            'actions': [],
            'lifecycle_frozen': self._lifecycle_frozen,
        }
        ...
        if self._step_count % self.config.check_interval == 0:
            ...
            if loss_diverging:
                ...
                if self.config.freeze_on_divergence and not self._lifecycle_frozen:
                    ...
                    logger.warning(
                        f"Freezing lifecycle due to loss divergence (freeze #{self._freeze_count})"
                    )
    ...
    def should_allow_lifecycle_changes(self) -> bool:
        """Check if lifecycle changes (births/deaths) are allowed"""
        return not self._lifecycle_frozen

    def unfreeze_lifecycle(self) -> None:
        """Manually unfreeze lifecycle"""
        if self._lifecycle_frozen:
            logger.info("Unfreezing lifecycle")
            self._lifecycle_frozen = False

    def force_cull_pool(self, pool, target_size: Optional[int] = None) -> int:
        """Force cull pool to target size.

        Args:
            pool: Pool to cull
            target_size: Target size (defaults to max_pool_size)

        Returns:
            Number of components culled
        """
    ...
    def cull_archive(self, archive, target_size: Optional[int] = None) -> int:
        """Cull archive to target size.

        Args:
            archive: Archive to cull
            target_size: Target size (defaults to max_archive_size)

        Returns:
            Number of elites removed
        """
        if target_size is None:
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            'config': {
                ...
                'max_archive_size': self.config.max_archive_size,
