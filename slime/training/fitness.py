...
class FitnessComputer:
    ...
    def compute_gradient_based_fitness(self, component: nn.Module, component_id: int) -> float:
        grad_norms = []
        ...
        if len(self._fitness_history[component_id]) == 1:
            ...
            smoothed_fitness = self.ema_alpha * raw_fitness + (1 - self.ema_alpha) * prev_fitness
        return smoothed_fitness

    def compute_attention_alignment_fitness(self, component: nn.Module, attention_weights: torch.Tensor, target_mask: torch.Tensor) -> float:
    ...
    def compute_information_bottleneck_fitness(self, component: nn.Module, input_activations: torch.Tensor, output_activations: torch.Tensor) -> float:
    ...
    def compute_compute_efficiency(self, component: nn.Module, component_id: int, forward_time_ms: float, baseline_time_ms: float=1.0) -> float:
        ...
        if len(self._latency_history[component_id]) == 1:
            ...
            smoothed_latency = self.ema_alpha * forward_time_ms + (1 - self.ema_alpha) * prev_latency
        efficiency = baseline_time_ms / (smoothed_latency + 1e-06)
        return min(efficiency, 1.0)

    def compute_combined_fitness(self, component: nn.Module, component_id: int, gradient_weight: float=0.1, attention_weights: Optional[torch.Tensor]=None, target_mask: Optional[torch.Tensor]=None, attention_weight: float=0.7, forward_time_ms: Optional[float]=None, efficiency_weight: float=0.2, input_activations: Optional[torch.Tensor]=None, output_activations: Optional[torch.Tensor]=None, information_weight: float=0.0) -> float:
        ...
        if attention_weight > 0 and attention_weights is not None and (target_mask is not None):
            attn_fitness = self.compute_attention_alignment_fitness(component, attention_weights, target_mask)
            fitness += attention_weight * attn_fitness
        if efficiency_weight > 0 and forward_time_ms is not None and self.include_efficiency:
            eff_fitness = self.compute_compute_efficiency(component, component_id, forward_time_ms)
        ...
        if information_weight > 0 and input_activations is not None and (output_activations is not None):
            info_fitness = self.compute_information_bottleneck_fitness(component, input_activations, output_activations)
    ...
    def stats(self) -> Dict:
        if not self._fitness_history:
            return {'num_components': 0, 'avg_fitness': 0.0, 'max_fitness': 0.0, 'min_fitness': 0.0}
        all_fitness = [f for history in self._fitness_history.values() for f in history]
        return {'num_components': len(self._fitness_history), 'total_computations': len(all_fitness), 'avg_fitness': sum(all_fitness) / len(all_fitness), 'max_fitness': max(all_fitness), 'min_fitness': min(all_fitness)}