"""Multi-objective loss functions for slime mold training"""
...
@dataclass
class LossWeights:
    """Weights for multi-objective loss components"""
...
class MultiObjectiveLoss(nn.Module):
    """Multi-objective loss for slime mold training.

    Combines task loss with architectural regularization to ensure:
    1. Task performance (reconstruction)
    2. Information flow (rank regularization)
    3. Stability (coherence regularization)
    4. Exploration (diversity)
    5. Archive quality (coverage)
    6. Lifecycle health (fitness variance)
    """
    ...
    def rank_regularization_loss(
        self,
        correlation_matrices: list[torch.Tensor]
    ) -> torch.Tensor:
        """Regularize effective rank to prevent collapse"""
    ...
    def coherence_regularization_loss(
        self,
        coherence_scores: list[torch.Tensor]
    ) -> torch.Tensor:
        """Regularize coherence to prevent instability"""
    ...
    def diversity_loss(
        self,
        pseudopod_outputs: list[torch.Tensor]
    ) -> torch.Tensor:
        """Encourage diversity among pseudopod outputs"""
    ...
    def archive_coverage_loss(
        self,
        archive_coverage: float,
        target_coverage: float = 0.2
    ) -> torch.Tensor:
        """Encourage better archive coverage"""
    ...
    def fitness_variance_loss(
        self,
        fitness_scores: list[float]
    ) -> torch.Tensor:
        """Encourage variance in fitness scores"""
    ...
    def forward(
        ...
        output: torch.Tensor,
        target: torch.Tensor,
        correlation_matrices: Optional[list[torch.Tensor]] = None,
        coherence_scores: Optional[list[torch.Tensor]] = None,
        pseudopod_outputs: Optional[list[torch.Tensor]] = None,
        ...
        total_loss = (
            self.weights.reconstruction * losses['reconstruction'] +
            self.weights.rank_regularization * losses['rank_regularization'] +
            self.weights.coherence_regularization * losses['coherence_regularization'] +
            self.weights.diversity * losses['diversity'] +
            self.weights.archive_coverage * losses['archive_coverage'] +
            self.weights.fitness_variance * losses['fitness_variance']
        )
