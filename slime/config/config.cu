...
// Behavioral tessellation dimensions (fixed for archive)
constexpr int BEHAVIORAL_DIM_HW = 8;
constexpr int BEHAVIORAL_DIM_TASK = 16;
constexpr int BEHAVIORAL_DIM_GEN = 8;
constexpr int BEHAVIORAL_DIM_TOTAL = BEHAVIORAL_DIM_HW + BEHAVIORAL_DIM_TASK + BEHAVIORAL_DIM_GEN;

constexpr size_t DIRESA_HW_STRIDE = HARDWARE_FEATURES_DIM * DIRESA_HIDDEN1_MAX + DIRESA_HIDDEN1_MAX + DIRESA_HIDDEN1_MAX * DIRESA_HIDDEN2_MAX + DIRESA_HIDDEN2_MAX + DIRESA_HIDDEN2_MAX * BEHAVIORAL_DIM_HW + BEHAVIORAL_DIM_HW + BEHAVIORAL_DIM_HW * DIRESA_HIDDEN2_MAX + DIRESA_HIDDEN2_MAX + DIRESA_HIDDEN2_MAX * DIRESA_HIDDEN1_MAX + DIRESA_HIDDEN1_MAX + DIRESA_HIDDEN1_MAX * HARDWARE_FEATURES_DIM + HARDWARE_FEATURES_DIM;
constexpr size_t DIRESA_GEN_STRIDE = 1 * DIRESA_HIDDEN1_MAX + DIRESA_HIDDEN1_MAX + DIRESA_HIDDEN1_MAX * DIRESA_HIDDEN2_MAX + DIRESA_HIDDEN2_MAX + DIRESA_HIDDEN2_MAX * BEHAVIORAL_DIM_GEN + BEHAVIORAL_DIM_GEN + BEHAVIORAL_DIM_GEN * DIRESA_HIDDEN2_MAX + DIRESA_HIDDEN2_MAX + DIRESA_HIDDEN2_MAX * DIRESA_HIDDEN1_MAX + DIRESA_HIDDEN1_MAX + DIRESA_HIDDEN1_MAX * 1 + 1;
...
constexpr float AGENT_SOURCE_STRENGTH_MAX = 2.0f;
constexpr float AGENT_SOURCE_STRENGTH_BASE_MIN = 0.1f;
constexpr float AGENT_SOURCE_STRENGTH_BASE_MAX = 2.0f;
constexpr float AGENT_SOURCE_SIGMA_BASE_MIN = 1.0f;
constexpr float AGENT_SOURCE_SIGMA_BASE_MAX = 5.0f;
constexpr float BEHAVIORAL_FIELD_SIGMA_BASE_MIN = 0.05f;
constexpr float BEHAVIORAL_FIELD_SIGMA_BASE_MAX = 0.5f;
constexpr float CHEMICAL_DECAY_BASE_MIN = 0.8f;
constexpr float CHEMICAL_DECAY_BASE_MAX = 0.99f;
constexpr float DIFFUSIVITY_BASE_MIN = 0.01f;
constexpr float DIFFUSIVITY_BASE_MAX = 0.5f;
constexpr float REACTION_ORDER_MIN = 1.0f;
constexpr float REACTION_ORDER_MAX = 3.0f;
constexpr float REACTION_RATE_MIN = 0.0f;
constexpr float REACTION_RATE_MAX = 0.5f;
constexpr float DECAY_RATE_MIN = 0.8f;
constexpr float DECAY_RATE_MAX = 0.99f;
constexpr float ATTRACTOR_SIGMA_BASE_MIN = 0.05f;
constexpr float ATTRACTOR_SIGMA_BASE_MAX = 0.3f;
...
// Shared architecture dimensions (environmental)
constexpr int WMMA_ALIGNMENT = 8;
constexpr int NUM_HEADS = 2;       // DEBUG: e2e (prod: 8)
...
constexpr size_t DIRESA_TASK_STRIDE_PER_ENTRY =
    ...
    DIRESA_HIDDEN2_MAX * BEHAVIORAL_DIM_TASK + BEHAVIORAL_DIM_TASK +
    BEHAVIORAL_DIM_TASK * DIRESA_HIDDEN2_MAX + DIRESA_HIDDEN2_MAX +
    DIRESA_HIDDEN2_MAX * DIRESA_HIDDEN1_MAX + DIRESA_HIDDEN1_MAX +
    DIRESA_HIDDEN1_MAX * DIRESA_TASK_INPUT_DIM + DIRESA_TASK_INPUT_DIM;

constexpr int FLOW_FIELD_DIMS = 2;
constexpr int AFFINITY_REDUCED_DIMS = 1;

// Derived sizes from environmental constants
constexpr int CA_FIELD_SIZE = GRID_SIZE * GRID_SIZE;
constexpr int CA_CONCENTRATION_SIZE = CA_FIELD_SIZE * CHANNELS;
constexpr int CA_OUTPUT_SIZE = CA_FIELD_SIZE * NUM_HEADS * HEAD_DIM;
...
constexpr int CA_REINTEGRATION_SIZE = CA_FIELD_SIZE * CHANNELS;
constexpr int CA_STATE_STRIDE = CA_CONCENTRATION_SIZE + CA_OUTPUT_SIZE + CA_AFFINITY_SIZE + CA_FLOW_SIZE + CA_REINTEGRATION_SIZE;

constexpr int CA_PERCEPTION_WEIGHT_SIZE = NUM_HEADS * CHANNELS * HEAD_DIM;
constexpr int CA_INTERACTION_WEIGHT_SIZE = NUM_HEADS * HEAD_DIM * HEAD_DIM;
constexpr int CA_FLOW_PROJECTION_WEIGHT_SIZE = NUM_HEADS * 2 * HEAD_DIM;
constexpr int CA_WEIGHTS_PER_ENTRY_STRIDE = CA_PERCEPTION_WEIGHT_SIZE + CA_INTERACTION_WEIGHT_SIZE + CA_FLOW_PROJECTION_WEIGHT_SIZE;

// Training batch size (environmental)
constexpr int BATCH_SIZE = 8;  // DEBUG: e2e (prod: 16)
constexpr int DATASET_SIZE_MAX = OPPORTUNITY_TRAIN_SAMPLES;

constexpr int SAVED_ACTIVATION_SIZE = BATCH_SIZE * NUM_HEADS * CA_FIELD_SIZE * HEAD_DIM;

constexpr int TAPE_ENTRIES_PER_ENTRY = TAPE_CAPACITY;
constexpr int TAPE_VALUES_PER_ENTRY = VALUE_CAPACITY;

constexpr int COL_WIDTH = 9 * CHANNELS;
constexpr int BACKWARD_CHUNK_SAMPLES = 256;  // DEBUG: e2e (prod: 1024)
constexpr size_t BACKWARD_WS_FP16_A_BLOCK = (size_t)NUM_HEADS * BACKWARD_CHUNK_SAMPLES * HIDDEN_DIM * sizeof(half);
constexpr size_t BACKWARD_WS_FP16_B_BLOCK = (size_t)NUM_HEADS * BACKWARD_CHUNK_SAMPLES * HIDDEN_DIM * sizeof(half);
constexpr size_t BACKWARD_WS_DW_BLOCK = (size_t)NUM_HEADS * HIDDEN_DIM * HIDDEN_DIM * sizeof(float);
constexpr size_t BACKWARD_WS_DI_BLOCK = (size_t)NUM_HEADS * BACKWARD_CHUNK_SAMPLES * HIDDEN_DIM * sizeof(float);
constexpr size_t BACKWARD_WS_W_T_BLOCK = (size_t)HIDDEN_DIM * HIDDEN_DIM * sizeof(half);  // Match dynamic: hidden_dim^2
constexpr size_t BACKWARD_WS_IM2COL_BLOCK = (size_t)CHANNELS * CA_FIELD_SIZE * CA_KERNEL_CELL_COUNT * sizeof(float);  // channels * cells * kernel_cells
constexpr size_t BACKWARD_WS_DPREGELU_BLOCK = (size_t)BACKWARD_CHUNK_SAMPLES * NUM_HEADS * CA_FIELD_SIZE * HEAD_DIM * sizeof(float);  // Match dynamic
...
// Environmental constants (field initial conditions)
constexpr float RESOURCE_INIT = 1.0f;
constexpr float RESOURCE_NOISE = 0.15f;
constexpr float CHEM_INIT_BASE = 0.5f;
constexpr float CHEM_INIT_NOISE = 0.1f;
