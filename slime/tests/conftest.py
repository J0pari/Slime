...
class ConstraintNode:
    ...
    timestamp: str
    parents: List[str] = field(default_factory=list)  # Causal predecessors
    children: List[str] = field(default_factory=list)  # Causal descendants
    edge_types: Dict[str, CausalEdgeType] = field(default_factory=dict)  # parent_id -> edge_type
    depth: int = 0  # Depth in causal DAG
...
class CausalConstraintChecker:
    def __init__(self, test_item):
        ...
        self.parent_stack: List[str] = []  # Stack for tracking causal parent during nested checks
    ...
    def check(
        self,
        constraint: str,
        satisfaction_probe: Callable[[], Tuple[bool, Any, Any, Dict]],
        context: Optional[Dict] = None,
        edge_type: CausalEdgeType = CausalEdgeType.REQUIRES
    ) -> Either[ConstraintViolation, Any]:
        """
        Check constraint and record in causal DAG.

        Returns Either monad:
        - Left(violation) if constraint violated
        - Right(actual_value) if constraint satisfied
        """
        ...
        # Push current constraint as potential parent for nested checks
        self.parent_stack.append(node_id)
        self.current_depth += 1

        try:
            # Execute satisfaction probe
    ...
    def kleisli_compose(
        self,
        f: Callable[[A], Either[ConstraintViolation, B]],
        g: Callable[[B], Either[ConstraintViolation, C]]
    ) -> Callable[[A], Either[ConstraintViolation, C]]:
        """
        Kleisli composition: (a -> Either e b) -> (b -> Either e c) -> (a -> Either e c)

        Composes two constraint checks sequentially with proper causal tracking.
        """
            ...
            'nodes': [
                {
                    ...
                    'expected': self._make_serializable(node.expected),
                    'actual': self._make_serializable(node.actual),
                    'details': self._make_serializable(node.details),
                    'context': self._make_serializable(node.context),
    ...
    def get_root_cause(self) -> Optional[ConstraintNode]:
        ...
        # DFS to find deepest violation in causal chain
        def depth_first_violation(node_id: str, visited: set) -> Optional[ConstraintNode]:
            ...
            if not node.satisfied:
                # Check parents first (causal predecessors)
        ...
        return violated[0]  # Fallback to first violation
...
def constraint(request):
    ...
    # Return checker's check method directly
...
def pytest_runtest_makereport(item, call):
    ...
    if report.when == 'call':
        ...
        # Extract causal DAG from constraint checker
        if hasattr(item, 'constraint_checker'):
            checker = item.constraint_checker
            test_result['causal_dag'] = checker.get_causal_dag()

            # Find root cause if any violations
            root_cause = checker.get_root_cause()
            if root_cause:
                test_result['root_cause'] = {
                    'constraint': root_cause.constraint,
                    'expected': root_cause.expected,
                    'actual': root_cause.actual,
                    'depth': root_cause.depth,
                    'causal_chain': root_cause.parents
                }

            # TRUTHFULNESS INVARIANT: Any unsatisfied constraint forces test failure
    ...
    for item in session.items:
        if hasattr(item, 'test_results'):
            for result in item.test_results:
                ...
                # Count constraints from causal DAG
    ...
    if results:
        ...
        summary = {
            ...
            'constraint_summary': {
                ...
                'avg_causal_depth': total_dag_depth / len([r for r in results if r.get('causal_dag')]) if results else 0.0
            },
            'results': results
        }

        # TRUTHFULNESS INVARIANT: Constraint violations force test failures
