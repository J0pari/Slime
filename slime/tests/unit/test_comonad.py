...
def test_spatial_context_initialization():
    ...
    ctx = SpatialContext(focus=focus, neighborhood=neighbors, distance_fn=behavioral_distance)
...
def test_get_k_nearest():
    ...
    neighbors = [MockComponent('near_1', behavior=torch.tensor([0.51, 0.51, 0.51, 0.51, 0.51])), MockComponent('near_2', behavior=torch.tensor([0.52, 0.52, 0.52, 0.52, 0.52])), MockComponent('far', behavior=torch.tensor([0.9, 0.9, 0.9, 0.9, 0.9]))]
    ctx = SpatialContext(focus=focus, neighborhood=neighbors, distance_fn=behavioral_distance)
...
def test_get_k_nearest_empty_neighborhood():
    ...
    ctx = SpatialContext(focus=focus, neighborhood=[], distance_fn=behavioral_distance)
...
def test_behavioral_distance():
    ...
    expected = 5 ** 0.5
    assert abs(distance - expected) < 1e-05
...
def test_extract_behavioral_divergence():
    ...
    neighbors = [MockComponent(f'neighbor_{i}', behavior=torch.tensor([0.5, 0.5, 0.5, 0.5, 0.5])) for i in range(5)]
    ctx = SpatialContext(focus=focus, neighborhood=neighbors, distance_fn=behavioral_distance)
...
def test_extract_behavioral_divergence_similar_neighbors():
    ...
    neighbors = [MockComponent(f'neighbor_{i}', behavior=behavior + torch.randn(5) * 0.01) for i in range(5)]
    ctx = SpatialContext(focus=focus, neighborhood=neighbors, distance_fn=behavioral_distance)
    divergence = extract_behavioral_divergence(ctx)
...
def test_extract_attention_coherence_high():
    ...
    neighbors = [MockComponent(f'neighbor_{i}', attention_pattern=pattern + torch.randn(1, 1, 16, 16) * 0.1) for i in range(5)]
    ctx = SpatialContext(focus=focus, neighborhood=neighbors, distance_fn=behavioral_distance)
...
def test_extract_attention_coherence_low():
    ...
    neighbors = [MockComponent(f'neighbor_{i}', attention_pattern=torch.randn(1, 1, 16, 16)) for i in range(5)]
    ctx = SpatialContext(focus=focus, neighborhood=neighbors, distance_fn=behavioral_distance)
    coherence = extract_attention_coherence(ctx)
...
def test_multiple_extractors_composition():
    ...
    neighbors = [MockComponent(f'neighbor_{i}', fitness=0.5, gradient_norm=2.0) for i in range(5)]
    ctx = SpatialContext(focus=focus, neighborhood=neighbors, distance_fn=behavioral_distance)
    relative_fitness = extract_relative_fitness(ctx)
