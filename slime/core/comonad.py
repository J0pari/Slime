...
def extract_relative_fitness(ctx: SpatialContext) -> float:
    ...
    std_neighbor_fitness = (
        sum((f - mean_neighbor_fitness) ** 2 for f in neighbor_fitnesses) / len(neighbor_fitnesses)
    ) ** 0.5

    if std_neighbor_fitness < 1e-6:
        return 0.0

    relative_fitness = (my_fitness - mean_neighbor_fitness) / (std_neighbor_fitness + 1e-6)
