...
The plan remains the normative delivery sequence. Current implementation and
verification evidence are tracked in [IMPLEMENTATION_STATUS.md](IMPLEMENTATION_STATUS.md)
so progress can be recorded without rewriting the wave requirements. The source
has advanced through the classifier-only integration and batched-backward work:

| Wave | Source status | Acceptance status |
| :--- | :------------ | :---------------- |
| 1 | Implemented | Revalidated on current source: 8/8 GPU checks passed |
| 2 | Implemented classifier-only baseline | Revalidated on current source: multi-generation run completed; Gates 1–3 transaction/evolution repairs landed |
| 2.5 | Implemented in source | Fresh build and regression pass; timing criterion still failed |
| 3 | Partial | Placeholder host path exists; required probe and GPU acceptance work remains |
| 4–8 | Not implemented | Not started |

The actionable work that follows from this plan and the engineering/behavioral
specifications is maintained in [../TODO.md](../TODO.md).

## Wave-to-claim projection

Each wave activates a set of normative claims (defined in blueprint.md and
cuda_engineering.md; validated by `architecture/compiler.py`). A wave's
acceptance is the set of its claims reaching mechanism-completeness with
their strong witnesses passing and evidence current.

- Wave 1: `A103.checkpoint-replay-equivalence`, `A501.came-production-equation`,
  `G100.single-prng`, `G100.deterministic-seed`.
- Wave 2: `A201.role-canonicalization`, `I001.spawn-wave-unique`,
  `I001.phase-order`, `S001.cuda-errors-fatal`, `S002.host-authority`,
  `S004.pt-swap-transaction`.
- Wave 2.5 (gates 1–3 repair): `A301.genotype-causes-phenotype`,
  `A103.gradient-correctness`, `A401.archive-genotype-attribution`,
  `I001.replay-evaluation-identity`, `A202.rd-disabled-until-adjoint`,
  `C001.acceptance-evidence-current`.
- Wave 4 (predictor activation): `A201.shared-substrate` reaches
  `established`, plus new predictor-role claims yet to be added.
- Wave 5 (structural pressures): `A401.bin-capacity`,
  `A401.live-statistics-exact`, `S002.operator-command-effective`,
  `A101.sot-schedule-independent` reach `established` with witnesses.

The claim registry is authoritative for status; this table only projects
waves onto claims and does not duplicate acceptance detail.

---

## Wave 1 — Autodiff + CAME (implemented; revalidation pending)

**Implemented scope**: `autodiff/warp_tape.cu` and `optimizer/came.cu` implement
the required functionality from `cuda_engineering.md` sections 4.1–4.3, 4.7,
4.8, 8, 10. Re-run this acceptance test against the current source before
claiming the wave verified.
...
## Wave 2 — Integration Layer + Classifier-Only Loop (implemented; acceptance pending)

**Implemented scope**: `integration/main_loop.cu`, `integration/host_main.cu`,
and `safety/alignment.cu` contain the classifier-only integration path for
`cuda_engineering.md` sections 2, 3, 5, 6, 9, 9.1, 12, 13, 15. Its full Wave 2
...
## Wave 2.5 — Batched Backward (implemented in source; validation pending)

**Implemented scope**: The source replaces the sequential cooperative-kernel
backward pass with a
...
## Wave 3 — Placeholder Regressor + Probe Set (partial)
...
- A wave cannot be marked complete while any function in that wave's scope is
  declared without a body. Future-wave declarations must remain clearly marked
...
- The active backward implementation uses the Wave 2.5 phase-decomposed batched
  sub-kernels (`<<<POOL_SIZE, 256>>>`) with the gather-based stencil adjoint.
  It uses no cooperative groups, `cudaLaunchCooperativeKernel`, or grid-wide
  synchronization. This supersedes the Wave 1 prototype requirement.
